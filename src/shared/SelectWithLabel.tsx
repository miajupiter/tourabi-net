      ...
      <>
        {id &&
          <div key={key || ('div-' + id)} className={`relative ${className}`}>
            <select
              id={id}
              className={`nc-Select block ${sizeClass} ${className}  w-full text-sm rounded-[4px] border-neutral-200 focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 bg-white dark:border-neutral-700 dark:focus:ring-primary-6000 dark:focus:ring-opacity-25 dark:bg-neutral-900`}
              {...args} >
              {children}
            </select>
